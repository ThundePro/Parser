...
class Product:
    ...
    def __init__(self, name, version, files=None, zip_file=None):
        if name == '4024':
            name = 'GXW' + name
            version = '1.0.12.4'
        elif name == 'BT200':
...
def unzip_files(array):
    for prod in array:
        with zipfile.ZipFile(prod.model + '.zip', 'r') as zip_ref:
            ...
            if len(file_names) > 5:
                ...
                for file in file_names:
                    try:
                        ...
                        pass
        try:
            os.remove(prod.model + '.zip')
        except PermissionError:
            shutil.rmtree(prod.model + '.zip')
        except FileNotFoundError:
            pass
    return array
...
lift_for_json = unzip_files(clear_list)
voip_dict = {
    'voip': [
        {
            "model": obj.model,
...
#
# deserialized_product = [Product(item_dict['model'], item_dict['version'], item_dict['files'])
